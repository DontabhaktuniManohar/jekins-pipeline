...
def library_version(server, folder, jobname):
    job_exists = False
    flodder = "%s" % (jobname)
    def _job_exists():
        print "check %s" % (flodder)
        return server.job_exists(flodder)
    job_exists = do_retry(_job_exists)
    print "job_exists(%s)" % (job_exists)
    if job_exists == True:
        ...
        __analyze_jobs(server,jobs)



def __analyze_jobs(server,jobs):
    for flodder in jobs:
        if flodder['_class'] == 'com.cloudbees.hudson.plugins.folder.Folder':
            xml = server.get_job_config(flodder['fullname'])
            response = xmltodict.parse(xml)
            #print  "%s" % (response)
            #print "---> %s" % (str(type(response.keys())))

            if type(response.keys()) == list:
                __analyze_jobs(server,response)
            else:
                try:
                    print "%-40s : %s (%s)" % (flodder['fullname'],
                        response['com.cloudbees.hudson.plugins.folder.Folder']['properties']['org.jenkinsci.plugins.workflow.libs.FolderLibraries']['libraries']['org.jenkinsci.plugins.workflow.libs.LibraryConfiguration']['retriever']['scm']['remote'],
                        response['com.cloudbees.hudson.plugins.folder.Folder']['properties']['org.jenkinsci.plugins.workflow.libs.FolderLibraries']['libraries']['org.jenkinsci.plugins.workflow.libs.LibraryConfiguration']['defaultVersion']
                        )
                except:
                    # print response
                    # print "%-40s : %s (%s)" % (flodder['fullname'],
...
def recreate_jobs(server, folder, jobname):
    ...
    def _job_exists():
        return server.job_exists(folder + jobname)
    job_exists = do_retry(_job_exists)
    if job_exists == True:
        my_job = ''
        ...
        def _delete_job():
            print " > deleting: %s" % (str(folder + jobname))
            my_ret = server.delete_job(folder + jobname)

        def _create_job():
            _file = open('delete_tmp.txt', 'r')
            my_job = _file.read()
            my_ret = server.create_job(folder + jobname, my_job)
            print(" > copied=%s" % (folder + jobname))
        stage = 'init'
        try:
            do_retry(_copy_job)
            stage = 'copy'
        ...
        if os.path.isfile('job.txt'):
            ...
            try:
                def _create_job():
                    ...
                    my_job = _file.read()
                    my_ret = server.create_job(folder + jobname, my_job)
                    print(" > created=%s from from file job.txt" %
                          (folder + jobname))
                do_retry(_create_job)
            except RetryException as err:
                print " ! skipping job %s after %s stage." % (jobname, stage)
...
server = jenkins.Jenkins('https://jenkins-shipment.web.fedex.com:8443/jenkins',
                         username='751818', password=os.environ['SC_PASSWORD'])
...
for job in list:
    ...
    if job.startswith('//') or job.startswith('#') or job == '':
        continue
    print "> %s" % (job)
    library_version(server,folderCurrent,job)
    ...
    # # job action
    # # recreate_jobs(server,folderCurrent,job)
