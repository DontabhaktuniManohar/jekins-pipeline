...
def call(body) {
   ...
   try {
        value = "${RUN_STATIC}"
        def bwcom = "./bw5cs.sh -s ${APP_EAR_LOC}/*.ear -t excel -o ${WORKSPACE}"
        bwcom = '''cd /opt/fedex/tibco/bw5cs2.2.2/bin
        ''' + bwcom
        goals[1] = [ 'name' : 'Static', 'goal' : resolveEnvironmentVar(bwcom,value), 'operation' : 'sh' ]
